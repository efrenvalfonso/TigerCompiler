...
{
    internal abstract class ArithmeticNode : NoShortCircuitBinaryNode
    {
        protected ArithmeticNode(IToken token)
            : base(token)
        {
        }
        ...
        public override bool CheckSemantic(Scope scope, List<Error> errors)
        {
            ...
            if (lvChS && !(LeftValue.NodeType is IntType))
            {
                ...
                if (!(LeftValue.NodeType is ErrorType))
                    errors.Add(
                        new Error(string.Format("Cannot implicitly convert type '{0}' to 'int'", LeftValue.NodeType),
            ...
            if (rvChS && !(RightValue.NodeType is IntType))
            {
                ...
                if (!(RightValue.NodeType is ErrorType))
                    errors.Add(
                        new Error(string.Format("Cannot implicitly convert type '{0}' to 'int'", RightValue.NodeType),
